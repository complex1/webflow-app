...
Some web-flow endpoints carry large `@swagger` blocks in controller modules as well as route-level JSDoc elsewhere. When you change a path or payload shape, update **one** canonical description (prefer route files or shared schema components in `src/config/swagger.ts`) so OpenAPI does not drift. Long-term, consolidate per-route docs into a single layer.
