<template>
  <Modal :open="open" title="Delete web flow" @close="$emit('close')">
    <Stack gap="sm">
      <Text tone="secondary" variant="sm">
        Are you sure you want to delete <strong>{{ webflow?.name }}</strong>? This action cannot be undone.
      </Text>
      <Inline gap="sm" justify="flex-end">
        <Button variant="ghost" type="button" @click="$emit('close')">Cancel</Button>
        <Button variant="danger" type="button" @click="$emit('confirm')">Delete</Button>
      </Inline>
    </Stack>
  </Modal>
</template>
...
defineProps<{ open: boolean; webflow: WebFlow | null }>()
