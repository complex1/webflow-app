<template>
  <AppLayout>
    <div class="envfile-container">
      ...
      <div class="envfile-list">
        <EnvfileCard
          v-for="envFile in filteredEnvFileList"
          :key="envFile.id"
          :envFile="envFile"
          @edit="handleEdit(envFile)"
          @delete="handleDelete(envFile)"
          @duplicate="handleDuplicate(envFile)"
        />
      </div>
    </div>
  </AppLayout>
  <UiDrawer
    v-model:visible="openAddEnvFileDrawer"
    title="Add Env File"
    position="right"
    size="lg"
    closable
    ...
    persistent
    overlay
    @close="openAddEnvFileDrawer = false"
  >
    <EnvfileForm
      :visible="openAddEnvFileDrawer"
      @submit="handleSubmit"
      @close="openAddEnvFileDrawer = false"
    />
  </UiDrawer>
  <UiDrawer
    v-model:visible="openEditEnvFileDrawer"
    title="Edit Env File"
    position="right"
    size="lg"
    closable
    ...
    persistent
    overlay
    @close="openEditEnvFileDrawer = false"
  >
    <EnvfileForm
      :visible="openEditEnvFileDrawer"
      @submit="handleSubmit"
      @close="openEditEnvFileDrawer = false"
  ...
  <UiModal
    v-model:visible="openDuplicateEnvFileModal"
    title="Duplicate Env File"
    position="right"
    size="lg"
    closable
    closeLabel="Close"
    @close="openDuplicateEnvFileModal = false"
  >
    <div class="duplicate-envfile-modal">
      <UiInput
        v-model="duplicateEnvFile.name"
        label="Name"
        placeholder="Enter name"
      />
      <UiButton variant="primary" @click="handleDuplicateEnvFile"
        >Duplicate</UiButton
      >
    </div>
  </UiModal>
...
import EnvfileForm, {
  type EnvFileFormData,
} from "@/components/features/envfile/envfileForm.vue";
...
const getEnvFiles = async () => {
  const response = await envFileService.getAll(
    envFilePagination.value.page,
    envFilePagination.value.limit
  );
...
const handleSearch = (query: string) => {
  filteredEnvFileList.value = envFileList.value.filter((envFile) =>
    envFile.name.toLowerCase().includes(query.toLowerCase())
  );
};
