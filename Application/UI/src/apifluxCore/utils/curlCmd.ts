...
/**
 * Parses a cURL command string and converts it to an HttpNode instance
 * @param curlCommand - The cURL command string to parse
 * @returns CurlParseResult containing the HttpNode or error information
 */
export function parseCurlToHttpNode(curlCommand: string): CurlParseResult {
  try {
    // Clean and normalize the curl command
    const cleanCommand = curlCommand.trim().replace(/\\\s*\n\s*/g, ' ');
    
    // Create new HttpNode instance
    ...
    httpNode.baseUrl.defaultValue = urlParts.baseUrl;
    httpNode.url.defaultValue = urlParts.path;
    ...
    urlParts.queryParams.forEach(param => {
      ...
      httpNode.queryParams.push(variable);
    });
    ...
    urlParts.pathParams.forEach(param => {
      ...
      httpNode.pathParams.push(variable);
    });
    
    // Extract HTTP method
    httpNode.method = extractMethod(cleanCommand);
    ...
    headers.forEach(header => {
      ...
      httpNode.headers.push(variable);
    });
    
    // Extract body data
    const bodyData = extractBody(cleanCommand);
    if (bodyData) {
      httpNode.body.defaultValue = bodyData;
    }
    
    // Set a default name for the node
    httpNode.name = 'Unnamed API Node';
    
    return { success: true, httpNode };
...
/**
 * Converts an HttpNode back to a cURL command string
 * @param httpNode - The HttpNode instance to convert
 * @param globalStore - Global store for variable resolution
 * @returns cURL command string
 */
export function httpNodeToCurl(httpNode: any, globalStore: Record<string, any> = {}): string {
  try {
    const url = httpNode.getUrl(globalStore);
    const headers = httpNode.getHeaders(globalStore);
    const body = httpNode.getBody(globalStore);
    
    let curlCommand = `curl -X ${httpNode.method}`;
    ...
    if (body && !['GET', 'HEAD', 'DELETE'].includes(httpNode.method)) {
    ...
    console.error('Error converting HttpNode to cURL:', error);
    return `curl -X ${httpNode.method} "${httpNode.baseUrl.defaultValue}${httpNode.url.defaultValue}"`;
  }
}
