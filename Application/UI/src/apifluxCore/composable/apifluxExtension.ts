...
import TransformNode from "../classes/transformNode";
...
const nodeToConfig = (nodeData: any) => {
  ...
  if (nodeData.type === NodeType.API) {
    const httpNode = new HttpNode(nodeData.id);
    httpNode.deserialized(config);
    return httpNode;
  }
  if (nodeData.type === NodeType.TRANSFORM) {
    // Add TransformNode deserialization when implemented
    const transformNode = new TransformNode({ id: nodeData.id });
    transformNode.deserialized(config);
